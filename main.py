...
diretorio = fr'C:\Users\helie\Desktop\tcc'
...
tipy_object = ['TABLE', 'VIEW', 'FUNCTION', 'PROCEDURE', 'EVENT', 'TRIGGER', 'INDEX', 'CONSTRANGES']
...
def show_index(base,table):
    result = mysql.query(f"SELECT CONCAT( 'CREATE ', IF(NON_UNIQUE = 0, 'UNIQUE ', ''), 'INDEX ', INDEX_NAME, ' ON ', TABLE_NAME, ' (', GROUP_CONCAT(COLUMN_NAME ORDER BY SEQ_IN_INDEX SEPARATOR ', '), ');' ) AS create_index_ddl FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = '{base}' AND TABLE_NAME = '{table}' GROUP BY TABLE_NAME, INDEX_NAME, NON_UNIQUE;")
    return result
...
for data in database:
    ...
    for amb in ambientes:
        ...
        for tab in table:
            ...
            index = show_index(data[0],tab[0])
            name_index = []
            if amb.find('5 - PRODUCTION') > -1:
                if not os.path.isfile(folder_database+f'\INDEX\{data[0]}'):
                    folder_index = f"{folder_database}\INDEX\{data[0]}"
                    folder_is_exists(folder_index)
                    file_index = open(f"{folder_index}\{tab[0]}.sql", 'w+')
                    for ind in index:
                        file_index.write(f"{ind[0]} \n")
